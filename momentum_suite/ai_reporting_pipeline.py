...
def classify_actionable_setups(df):
    """Cleanly preserves original momentum_revised.py tags and layers in High-Conviction signals."""
    ...
    for index, row in df.iterrows():
        ...
        h = row.get('High', 0)
        l = row.get('Low', 0)
        put_wall = row.get('Put_Wall_Floor', 0)
        call_wall = row.get('Call_Wall_Ceiling', 0)

        # Grab the exact tags generated by momentum_revised.py
        momentum = str(row.get('Momentum_Signal', '')).strip()
        regime = str(row.get('Market_Regime', '')).strip()
        
        # 1. Baseline: Use the original momentum signal or market regime from momentum_revised.py
        ...
        # 2. Optional: Refine with 0 DTE or Swing rules only if walls are valid numbers
        try:
            ...
            pass  # Fall back to the original momentum_revised tag if math fails

        strategy_tags.append(tag)

    df['Actionable_Strategy'] = strategy_tags
    
    # CRITICAL: Return every single row so nothing ever vanishes
    return df
